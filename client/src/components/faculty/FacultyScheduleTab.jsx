        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col gap-4 bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
                <div className="flex flex-col lg:flex-row justify-between items-center gap-4">
                    <div className="flex space-x-1 bg-slate-100 p-1 rounded-lg w-full lg:w-fit overflow-x-auto">
                        <button
                    ...
                    <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                        ...
                        <div className="relative flex-1 min-w-[200px]">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 ring-blue-500 outline-none transition-all text-sm"
                            />
                        </div>
                        ...
                        <select
                            ...
                            className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-bold text-slate-600 focus:ring-2 ring-blue-500 outline-none"
                        >
            ...
            <div className="grid gap-4">
                {loading ? (
                    <div className="py-20 flex flex-col items-center justify-center text-slate-400 bg-white rounded-lg border border-slate-100 shadow-sm">
                        <div className="w-8 h-8 border-4 border-blue-600 border-t-transparent rounded-full animate-spin mb-4" />
                        <p className="font-medium animate-pulse">Fetching your schedule...</p>
                    </div>
                ) : filteredSessions.length === 0 ? (
                    <div className="p-12 text-center text-slate-400 bg-white rounded-lg border-2 border-dashed border-slate-200">
                    ...
                    filteredSessions.map(session => (
                        <div key={session.id} className="bg-white p-6 rounded-lg border border-slate-200 shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
                            <div className="flex flex-col md:flex-row justify-between gap-6">

                                {/* Time & Venue */}
                                <div className="flex gap-4 items-start min-w-[240px]">
                                    <div className={`p-4 rounded-lg shadow-sm ${isSessionToday(session.startTime) ? 'bg-green-100 text-green-700' : 'bg-blue-50 text-blue-600'}`}>
                                        <Calendar size={28} />
                                    </div>
                                ...
                                <div className="flex-1 space-y-3">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-2">
                                        {(session.displayStudents || []).slice(((studentPages[session.id] || 1) - 1) * 50, (studentPages[session.id] || 1) * 50).map((student, sIdx) => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className="group flex items-center gap-3 p-2.5 bg-slate-50 rounded-lg border border-transparent hover:border-blue-200 hover:bg-white hover:shadow-md transition-all text-left"
                                                >
                                        ...
                                        {(!session.displayStudents || session.displayStudents.length === 0) && (
                                            <span className="text-sm text-slate-400 italic bg-slate-50 px-4 py-3 rounded-lg border border-dashed border-slate-200 w-full text-center sm:col-span-2 xl:col-span-3">
